...
def model_exports_dir(model: str) -> pathlib.Path:
    p = EXPORTS_DIR / model; p.mkdir(parents=True, exist_ok=True); return p
...
def read_csv_any(path: pathlib.Path):
    ...
    header = rows[0] if rows else []; data = rows[1:] if header else rows
    return header, data
...
def evidence_sanity_check(match: str, snippet: str, variable_name: str):
    ...
    Rules:
    - If snippet has no letters or length < 2 → blank snippet; If match==Yes → downgrade to Maybe.
...
class ReviewTable(ttk.Treeview):
    ...
    def load_from_pipeline_rows(self, rows6):
        ...
        self.delete(*self.get_children()); self.rows = []
        for r in rows6:
            r = (r + [""]*6)[:6]
    ...
    def set_rows_ui(self, ui_rows):
        self.delete(*self.get_children()); self.rows = [r[:] for r in ui_rows]
    ...
    def on_single_click(self, event):
        ...
        idx = int(row_id); col_idx = int(col_id.replace("#","")) - 1
    ...
    def export_comprehensive_xlsx(self, model: str):
        ...
        summary_ws.cell(1, 1, "Export Summary").font = Font(bold=True, size=14)
    ...
    def worker():
        try:
            ...
            if proc.returncode != 0:
                ...
                status_var.set("❌ Failed"); messagebox.showerror("Pipeline error", msg); return
            status_var.set("Loading results…")
            latest = find_latest_ticksheet(model)
            if not latest:
                status_var.set("❌ No results found"); messagebox.showerror("Not found", "No ticksheet found in exports."); return
            ...
            status_var.set("❌ Failed"); messagebox.showerror("Error", str(e))
        finally:
            run_btn.config(state=tk.NORMAL)
    run_btn.config(state=tk.DISABLED); threading.Thread(target=worker, daemon=True).start()
...
def main():
    ROOT.mkdir(exist_ok=True); MASTERLIST_DIR.mkdir(exist_ok=True)
    SESSIONS_DIR.mkdir(exist_ok=True); EXPORTS_DIR.mkdir(exist_ok=True)
    models = list_models()
    ...
    menubar = tk.Menu(win); filemenu = tk.Menu(menubar, tearoff=0)
    ...
    def _export_copy_menu():
        ...
        count = table.copy_export_column(); messagebox.showinfo("Copied", f"Export column copied ({count} X marks)")
    def _export_csv_menu():
        if not table.rows: return messagebox.showwarning("Nothing to export", "Run analysis first.")
        out = table.export_reviewed_with_tick(model_var.get().strip()); messagebox.showinfo("Exported", f"CSV saved: {out.name}")
    ...
    menubar.add_cascade(label="File", menu=filemenu); win.config(menu=menubar)
    ...
    def on_load_file():
        ...
        if not path: return
        text_box.delete("1.0", tk.END); text_box.insert("1.0", pathlib.Path(path).read_text(encoding="utf-8", errors="ignore"))
    ...
    def on_run_intelligent():
        ...
        def worker():
            try:
                ...
                intelligent_btn.config(state=tk.DISABLED)
                ...
                intelligent_btn.config(state=tk.NORMAL)
    ...
    def on_run_mega():
        ...
        def worker():
            try:
                ...
                mega_btn.config(state=tk.DISABLED)
                ...
                mega_btn.config(state=tk.NORMAL)
    ...
    def on_run_consensus():
        ...
        llm_model = llm_var.get().strip()
        consensus_btn.config(state=tk.DISABLED)
        ...
        def worker():
            ...
            try:
                ...
                consensus_btn.config(state=tk.NORMAL)
    ...
    info_text = "💡 Quick Guide: 🚀 MEGA: LV→EN→3x consensus • 🎓 Learning: Enhanced with learned knowledge • ✨ Smart: 3x consensus • 📚 Dictionary: Search & edit features • Press F11 for fullscreen, ESC to exit"
    ...
    bottom = tk.Frame(win); bottom.pack(fill="x", padx=10, pady=(0,10))
    ...
    def on_export_ticked():
        ...
        out = table.export_ticked_only(model_var.get().strip()); messagebox.showinfo("Exported", f"Saved {out.name}")
